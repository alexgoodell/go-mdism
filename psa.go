		...
		case "transition-probabilities":
			...
			for p := 0; p < len(Inputs.TransitionProbabilities); p++ {
				...
				if transitionProbability.PSA_id == psaInput.Id && transitionProbability.PSA_id != 0 {
					...
					fmt.Println()
					fmt.Println("psa: =======================")
					fmt.Printf("%+v", psaInput)
					fmt.Println()
					fmt.Println("tp =========================")
					fmt.Printf("%+v", transitionProbability)
					fmt.Println("old was ", transitionProbability.Tp_base)
					transitionProbability.Tp_base = psaInput.Value
					fmt.Println("new is ", transitionProbability.Tp_base)
				}
			}
