...
func setUpQueryData(numberOfPeople int) {
	...
	QueryData.Tps_id_by_from_state = make([][]int, len(States), len(States))
	for i, _ := range QueryData.Tps_id_by_from_state {
		...
		QueryData.Tps_id_by_from_state[i] = tPIdsToReturn
	}
...
func check_sum(theseTPs []TransitionProbability) {
	...
	if !equalFloat(sum, 1.0, 0.000000001) {
...
func (state *State) get_destination_probabilites() []TransitionProbability {
	...
	tPIdsToReturn = QueryData.Tps_id_by_from_state[state.Id]

	for _, id := range tPIdsToReturn {
		tPsToReturn = append(tPsToReturn, TransitionProbabilities[id])
	}
